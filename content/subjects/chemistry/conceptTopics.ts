...
interface ConceptChapterDefinition {
  key: SectionKey;
  id: string;
  title: string;
...
interface ConceptUnitSeed {
  title: string;
  aliases?: string[];
  keywords?: string[];
}

interface ConceptTopicSeed {
  id: string;
  title: string;
  description: string;
  units: ConceptUnitSeed[];
}

interface ConceptUnitContext {
  id: string;
  subjectId: string;
  topicId: string;
  topicTitle: string;
...
const chapterDefinitions: ConceptChapterDefinition[] = [
  {
    key: 'classification',
...
function seed(title: string, aliases: string[] = [], keywords: string[] = []): ConceptUnitSeed {
  return { title, aliases, keywords };
}

const conceptTopicSeeds: ConceptTopicSeed[] = [
  {
    id: 'atomic-structure-periodicity',
    title: 'Atomic Structure & Periodicity',
    description:
      'Foundational units on atomic models, quantum description, electron arrangement, and the periodic logic used to organize chemical behavior.',
    units: [
  ...
  },
  {
    ...
    description:
      'A structured bonding sequence covering ionic, covalent, metallic, and intermolecular models used to explain molecular and solid-state structure.',
    units: [
  ...
  },
  {
    ...
    description:
      'Core quantitative chemistry on formulas, equations, mole relationships, reaction classes, and calculation frameworks used across the laboratory.',
    units: [
  ...
  },
  {
    ...
    description:
      'Energy-centered chemistry content on heat, enthalpy, entropy, free energy, state functions, and phase change interpretation.',
    units: [
  ...
  },
  {
    ...
    description:
      'Units on reaction speed, rate laws, activation barriers, mechanisms, and catalysis, framed around the logic of chemical change over time.',
    units: [
      seed('Reaction Rates'),
  ...
  },
  {
    ...
    title: 'Equilibrium',
    description:
      'An equilibrium sequence covering mass action, equilibrium constants, ICE tables, solubility systems, and response to changing conditions.',
    units: [
  ...
  },
  {
    ...
    description:
      'Solution chemistry content on acid-base theory, aqueous equilibrium, buffers, titrations, solubility, and concentration relationships.',
    units: [
      ...
      seed(
        ...
        ['arrhenius', 'bronsted lowry', 'lewis acid', 'lewis base']
      ),
  ...
  },
  {
    ...
    description:
      'Electrochemical units on oxidation-reduction logic, cell potentials, electrolysis, batteries, corrosion, and quantitative charge relationships.',
    units: [
  ...
  },
  {
    ...
    description:
      'A broad organic sequence covering hydrocarbons, functional groups, stereochemistry, common reaction classes, biomolecular foundations, and polymer chemistry.',
    units: [
  ...
  },
  {
    ...
    description:
      'Analytical chemistry units on measurement quality, separation, titration, calibration, and instrumental analysis across classical and modern methods.',
    units: [
  ...
  },
  {
    ...
    description:
      'Materials-focused chemistry content on solids, band structure, functional materials, nanoscale systems, and the relationship between structure and performance.',
    units: [
...
const conceptAccentOffsets: Record<string, number> = {
...
function lowerConcept(context: Pick<ConceptUnitContext, 'title'>) {
  return context.title.toLowerCase();
}

function buildSummary(topic: ConceptTopicSeed, unit: ConceptUnitSeed) {
...
function buildOverview(topic: ConceptTopicSeed, unit: ConceptUnitSeed) {
...
function buildSections(context: Omit<ConceptUnitContext, 'sections'>): Record<SectionKey, SectionSeed> {
  const concept = lowerConcept(context);

  return {
    classification: section(
      `${context.title} is introduced as a core idea within ${context.topicTitle}, so the opening chapter defines the term, sets boundaries, and establishes why chemists return to it throughout the course.`,
      [
    ...
    electronic: section(
      `This chapter treats ${context.title} through models, structure, and core relationships, because introductory chemistry becomes much clearer when the concept is tied to a stable explanatory picture.`,
      [
    ...
    physical: section(
      `Chemistry students meet ${context.title} not only in words but also through quantities, representations, and worked comparisons, so this chapter focuses on the forms in which the idea is actually handled.`,
      [
    ...
    reactivity: section(
      `The usefulness of ${context.title} becomes clearer once it is tied to governing principles such as energy change, particle interaction, equilibrium response, or electron transfer, depending on the system being discussed.`,
      [
    ...
    occurrence: section(
      `${context.title} appears across classroom examples, laboratory method, industrial chemistry, and natural systems, so the chapter treats context as part of chemical understanding rather than as an afterthought.`,
      [
    ...
    isotopes: section(
      `Beyond definition and vocabulary, ${context.title} has a quantitative and theoretical side. This chapter explains the logic behind the formulas, assumptions, and deeper consequences that make the topic transferable to new problems.`,
      [
    ...
    production: section(
      `Students operationalize ${context.title} through worked examples, measurements, and repeatable problem-solving workflows, so this chapter is organized around method rather than isolated fact recall.`,
      [
    ...
    applied: section(
      `${context.title} matters because it shapes how chemists interpret experiments, build materials, control processes, or understand the environment. The closing chapter ties the concept to practical use, intellectual significance, and safe professional judgment.`,
      [
...
function buildParagraphs(context: ConceptUnitContext, definition: ConceptChapterDefinition, section: SectionSeed) {
  const concept = lowerConcept(context);
...
function buildFigureData(context: ConceptUnitContext, definition: ConceptChapterDefinition, section: SectionSeed): FigureContentBlock['figure'] {
  switch (definition.key) {
    case 'classification':
      return {
...
function buildFigureTitle(context: ConceptUnitContext, definition: ConceptChapterDefinition) {
  return `${context.title}: ${definition.figureLabel}`;
}

function buildFigureCaption(context: ConceptUnitContext, definition: ConceptChapterDefinition, section: SectionSeed) {
  return `${definition.title} for ${context.title} in ${context.topicTitle}. ${section.highlights[0]}`;
}

function buildFigureAltText(context: ConceptUnitContext, definition: ConceptChapterDefinition) {
  return `${definition.figureLabel} figure for ${context.title}.`;
}

function buildGlossaryItems(context: ConceptUnitContext, key: SectionKey, section: SectionSeed) {
  switch (key) {
    case 'classification':
      return [
...
function buildFactPrompts(context: ConceptUnitContext, key: SectionKey) {
  switch (key) {
    case 'classification':
      return [
...
function buildStatementFrames(context: ConceptUnitContext, key: SectionKey) {
  switch (key) {
    case 'classification':
      return [
    ...
    case 'physical':
      return [
        (fact: string) => `Key quantities for ${context.title} are expressed through ${fact}.`,
        (fact: string) => `A useful representation is ${fact}.`,
        (fact: string) => `Representative values show that ${fact}.`,
    ...
    case 'reactivity':
      return [
        ...
        (fact: string) => `Chemists rely on ${context.title} because ${fact}.`,
        (fact: string) => `A representative context is ${fact}.`,
    ...
    case 'occurrence':
      return [
        ...
        (fact: string) => `A chemist is likely to meet the topic through ${fact}.`,
        (fact: string) => `Context matters because ${fact}.`,
        (fact: string) => `A simplified treatment is acceptable when ${fact}.`,
      ];
    case 'isotopes':
      return [
        (fact: string) => `Quantitative chemistry treats ${context.title} through ${fact}.`,
    ...
    case 'production':
      return [
        ...
        (fact: string) => `Precision depends on ${fact}.`,
...
function buildUnitGlossary(context: ConceptUnitContext): GlossaryTerm[] {
  return [
    {
      term: context.title,
...
function collectSearchTerms(context: ConceptUnitContext) {
  return unique([
    context.title,
    context.shortCode,
    context.topicTitle,
...
function buildFlashcards(context: ConceptUnitContext, definition: ConceptChapterDefinition, section: SectionSeed): Flashcard[] {
...
function chooseDistractors(
  context: ConceptUnitContext,
  key: SectionKey,
  index: number,
  allContexts: ConceptUnitContext[]
) {
...
function buildQuiz(
  context: ConceptUnitContext,
  definition: ConceptChapterDefinition,
  section: SectionSeed,
  allContexts: ConceptUnitContext[]
): QuizQuestion[] {
  const prompts = buildFactPrompts(context, definition.key);
  ...
  const multipleChoiceQuestions = section.facts.map((fact, index) =>
    multipleChoice(
      ...
      `The best answer is "${formatStandaloneText(fact)}" because ${section.highlights[index % section.highlights.length].toLowerCase()}`,
      index
    )
  );
...
function createChapter(
  ...
  const blocks = [
    ...
    bullets(
      `${definition.id}-glossary`,
      'Glossary Focus',
      buildGlossaryItems(context, definition.key, section)
    ),
  ];
...
function createUnitContext(topic: ConceptTopicSeed, unit: ConceptUnitSeed, index: number): ConceptUnitContext {
  const order = index + 1;
...
function createConceptUnit(context: ConceptUnitContext, allContexts: ConceptUnitContext[]): LearningUnit {
  return {
    id: context.id,
...
const conceptUnitContexts = conceptTopicSeeds.flatMap((topic) =>
  topic.units.map((unit, index) => createUnitContext(topic, unit, index))
);

export const chemistryConceptAccentSources = Object.fromEntries(
  conceptUnitContexts.map((context) => {
...
export const chemistryConceptTopics: Topic[] = conceptTopicSeeds.map((topic) => {
...
export const chemistryConceptUnits = chemistryConceptTopics.flatMap((topic) => topic.learningUnits);
