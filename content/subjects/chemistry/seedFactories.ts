...
import type { SectionSeed } from '@/content/subjects/chemistry/elementSeeds';

export type FactSet = [string, string, string, string, string];

function section(focusNote: string, highlights: string[], facts: FactSet): SectionSeed {
  return { focusNote, highlights, facts };
}

export function term(entry: string, definition: string): GlossaryTerm {
  return { term: entry, definition };
}

export function classificationSection(name: string, facts: FactSet): SectionSeed {
  return section(
    `${name} is best introduced as ${facts[0]}, and its periodic position becomes most useful when tied directly to the compounds and ions it actually forms.`,
    [
...
export function electronicSection(name: string, facts: FactSet): SectionSeed {
  return section(
    `The valence picture of ${name} begins with ${facts[0]}, and that arrangement explains most of the bonding and oxidation behavior seen in ordinary compounds.`,
    [
...
export function physicalSection(name: string, facts: FactSet): SectionSeed {
  return section(
    `The physical form of ${name} is summarized as ${facts[0]}, and that form already reflects the structure and bonding forces characteristic of the element.`,
    [
...
export function occurrenceSection(name: string, facts: FactSet): SectionSeed {
  return section(
    `${name} is found mainly as ${facts[0]}, so its natural distribution is controlled by the forms in which the element is chemically stabilized.`,
    [
...
export function isotopeSection(name: string, facts: FactSet): SectionSeed {
  return section(
    `The isotope story of ${name} centers on ${facts[0]}, linking ordinary chemistry to nuclear stability, measurement, tracing, and sometimes radiological control.`,
    [
...
export function productionSection(name: string, facts: FactSet): SectionSeed {
  return section(
    `Modern supply of ${name} depends on ${facts[0]}, so industrial chemistry is inseparable from separation, reduction, purification, and energy cost.`,
    [
...
export function appliedSection(name: string, facts: FactSet): SectionSeed {
  return section(
    `${name} matters technologically through ${facts[0]}, where atomic-scale behavior becomes useful at industrial, environmental, biological, or laboratory scale.`,
    [
