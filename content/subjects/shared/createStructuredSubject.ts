...
export interface StructuredUnitSeed {
  title: string;
  aliases?: string[];
  keywords?: string[];
}

export interface StructuredTopicSeed {
  id: string;
  title: string;
  description: string;
  units: StructuredUnitSeed[];
}

export interface StructuredSubjectConfig {
  id: string;
  title: string;
  description: string;
  tagline: string;
  ...
  topics: StructuredTopicSeed[];
  accentCycle: readonly string[];
}
...
export const defaultSubjectAccentCycle = [
  'silicon',
  'oxygen',
  'calcium',
  'cobalt',
...
export function structuredUnits(entries: Array<string | StructuredUnitSeed>): StructuredUnitSeed[] {
...
function lowerConcept(context: Pick<UnitContext, 'title'>) {
  return context.title.toLowerCase();
}

function buildSummary(topic: StructuredTopicSeed, unit: StructuredUnitSeed, context: Pick<UnitContext, 'subjectLower'>) {
...
function buildOverview(topic: StructuredTopicSeed, unit: StructuredUnitSeed, context: Pick<UnitContext, 'subjectLower'>) {
...
function buildHeroFacts(topic: StructuredTopicSeed, order: number, totalUnits: number, shortCode: string) {
  return [
...
function inferDifficulty(order: number, totalUnits: number): ContentDifficulty {
...
function getTopicIndexById(topics: StructuredTopicSeed[], topicId: string) {
  return topics.findIndex((topic) => topic.id === topicId);
}

function buildSections(context: Omit<UnitContext, 'sections'>): Record<SectionKey, SectionSeed> {
  const concept = lowerConcept(context);
  const subject = context.subjectLower;

  return {
    classification: section(
      `${context.title} is introduced as a core concept within ${context.topicTitle}, so the opening chapter defines the term, sets scope, and explains why it matters within ${subject}.`,
      [
    ...
    electronic: section(
      `${context.title} becomes clearer when it is linked to models and system relationships, so this chapter explains the structural picture used to make the topic intelligible.`,
      [
    ...
    physical: section(
      `${context.title} is encountered not only in words but also through properties, measurements, and representations, so this chapter focuses on the forms in which the idea is handled in practice.`,
      [
    ...
    reactivity: section(
      `The usefulness of ${context.title} becomes clearer when it is tied to the processes or governing principles that make a system change, remain stable, or behave in a recognizable way.`,
      [
    ...
    occurrence: section(
      `${context.title} appears across classroom examples, interpreted evidence, practical workflows, and real-world settings, so the chapter treats context as part of understanding rather than an afterthought.`,
      [
    ...
    isotopes: section(
      `Beyond definition and description, ${context.title} has a quantitative and theoretical side. This chapter explains the logic behind formulas, assumptions, measurement tools, and deeper principles that make the topic transferable.`,
      [
    ...
    production: section(
      `Students operationalize ${context.title} through workflows, observation, data organization, and worked interpretation, so this chapter is organized around method rather than isolated fact recall.`,
      [
    ...
    applied: section(
      `${context.title} matters because it shapes how ${context.practitionerPlural} interpret evidence, assess systems, build explanations, or make practical decisions. The closing chapter ties the topic to application, broader significance, and responsible professional judgment.`,
      [
...
function buildParagraphs(context: UnitContext, definition: ChapterDefinition, seed: SectionSeed) {
  const concept = lowerConcept(context);
...
function buildFigureData(context: UnitContext, definition: ChapterDefinition, sectionSeed: SectionSeed): FigureContentBlock['figure'] {
  switch (definition.key) {
    case 'classification':
      return {
...
function buildGlossaryItems(context: UnitContext, key: SectionKey, sectionSeed: SectionSeed) {
  switch (key) {
    case 'classification':
      return [
...
function buildFactPrompts(context: UnitContext, key: SectionKey) {
  switch (key) {
    case 'classification':
      return [
...
function buildStatementFrames(context: UnitContext, key: SectionKey) {
  switch (key) {
    case 'classification':
      return [
    ...
    case 'reactivity':
      return [
        ...
        (fact: string) => `${context.practitionerPlural} rely on ${context.title} because ${fact}.`,
    ...
    case 'occurrence':
      return [
        ...
        (fact: string) => `A learner is likely to meet the topic through ${fact}.`,
        (fact: string) => `Context matters because ${fact}.`,
        (fact: string) => `A simplified treatment is acceptable when ${fact}.`,
      ];
    case 'isotopes':
      return [
        (fact: string) => `Quantitative study treats ${context.title} through ${fact}.`,
    ...
    case 'applied':
      return [
        ...
        (fact: string) => `Responsible practice requires that ${fact}.`,
      ];
  }
}

function buildUnitGlossary(context: UnitContext): GlossaryTerm[] {
  return [
    {
      term: context.title,
...
function collectSearchTerms(context: UnitContext) {
  return unique([
    context.title,
    context.shortCode,
    context.topicTitle,
...
function buildFlashcards(context: UnitContext, definition: ChapterDefinition, seed: SectionSeed): Flashcard[] {
...
function chooseDistractors(context: UnitContext, key: SectionKey, index: number, allContexts: UnitContext[]) {
...
function buildQuiz(context: UnitContext, definition: ChapterDefinition, seed: SectionSeed, allContexts: UnitContext[]) {
  const prompts = buildFactPrompts(context, definition.key);
  const statementFrames = buildStatementFrames(context, definition.key);

  const multipleChoiceQuestions = seed.facts.map((fact, index) =>
    multipleChoice(
      `${definition.id}-mc-${index + 1}`,
      prompts[index],
      fact,
      chooseDistractors(context, definition.key, index, allContexts),
      `The best answer is "${formatStandaloneText(fact)}" because ${seed.highlights[index % seed.highlights.length].toLowerCase()}`,
      index
    )
  );
...
function createChapter(context: UnitContext, definition: ChapterDefinition, allContexts: UnitContext[]): Chapter {
  ...
  const blocks = [
    ...
    figure(
      ...
      `${context.title}: ${definition.figureLabel}`,
      `${definition.title} for ${context.title} in ${context.topicTitle}. ${seed.highlights[0]}`,
      `${definition.figureLabel} figure for ${context.title}.`,
      buildFigureData(context, definition, seed)
    ),
...
function createUnitContext(
  config: StructuredSubjectConfig,
  topic: StructuredTopicSeed,
  topicIndex: number,
  unit: StructuredUnitSeed,
  index: number
): UnitContext {
  const order = index + 1;
...
function createLearningUnit(context: UnitContext, allContexts: UnitContext[]): LearningUnit {
...
export function createStructuredSubject(config: StructuredSubjectConfig) {
  const unitContexts = config.topics.flatMap((topic, topicIndex) =>
    topic.units.map((unit, index) => createUnitContext(config, topic, topicIndex, unit, index))
  );

  const accentSources = Object.fromEntries(
    unitContexts.map((context) => {
      const accentSource =
        config.accentCycle[(context.order - 1 + (context.topicOrder - 1) * 3) % config.accentCycle.length];
      return [context.id, accentSource];
    })
  ) as Record<string, string>;

  const topics: Topic[] = config.topics.map((topic) => {
