...
exports.geologyUnits = exports.geologyTopics = exports.geologyAccentSources = void 0;
...
const geologyTopicSeeds = [
    {
        id: 'foundations-of-geology',
        title: 'Foundations of Geology',
        description: 'Introductory geology concepts, the scope of Earth science, and the methods used to study Earth materials, processes, and history.',
        units: [
    ...
    },
    {
        id: 'minerals',
        title: 'Minerals',
        description: 'The building blocks of rocks, including mineral properties, classification, identification, and formation environments.',
        units: [
    ...
    },
    {
        ...
        description: 'How magma forms, evolves, and solidifies, and how igneous rocks are classified and interpreted.',
        units: [
            seed('Magma and Lava'),
    ...
    },
    {
        ...
        description: 'Weathering, erosion, transport, deposition, lithification, and the interpretation of sedimentary environments.',
        units: [
    ...
    },
    {
        ...
        description: 'How heat, pressure, fluids, and deformation alter rocks and create metamorphic textures and mineral assemblages.',
        units: [
    ...
    },
    {
        ...
        description: 'The internal structure of Earth and the large-scale tectonic processes that shape the planet.',
        units: [
    ...
    },
    {
        ...
        description: 'Deformation of rocks and the structures produced by stress, strain, and tectonic forces.',
        units: [
    ...
    },
    {
        id: 'earthquakes',
        title: 'Earthquakes',
        description: 'Seismic processes, fault motion, earthquake measurement, and associated hazards.',
        units: [
    ...
    },
    {
        id: 'volcanoes',
        title: 'Volcanoes',
        description: 'Volcanic systems, eruption styles, volcanic landforms, and related hazards.',
        units: [
    ...
    },
    {
        ...
        description: 'How running water, wind, ice, gravity, and climate shape Earth’s surface over time.',
        units: [
    ...
    },
    {
        ...
        description: 'How geologists reconstruct Earth history using relative dating, absolute dating, stratigraphy, and the fossil record.',
        units: [
    ...
    },
    {
        ...
        description: 'Ancient life, fossilization, biological change through time, and major transitions in Earth’s biosphere.',
        units: [
    ...
    },
    {
        ...
        description: 'The geology of ocean basins, seafloor processes, marine sediments, and ocean-Earth interactions.',
        units: [
    ...
    },
    {
        ...
        description: 'Geology as applied to natural resources, land use, hazards, water systems, and human-environment interaction.',
        units: [
    ...
    },
    {
        ...
        description: 'How geology supports extraction, exploration, engineering, and practical decision-making.',
        units: [
...
function buildSections(context) {
    ...
    return {
        classification: section(`${context.title} is introduced as a core geology concept within ${context.topicTitle}, so the opening chapter defines the term, sets scope, and explains why it matters for Earth materials, processes, or history.`, [
        ...
        electronic: section(`Geology becomes clearer when ${context.title} is linked to models and Earth-system relationships, so this chapter explains the structural picture that geologists use to make the topic intelligible.`, [
        ...
        physical: section(`${context.title} is encountered not only in words but also through properties, measurements, and visual representations, so this chapter focuses on the forms in which the idea is identified and communicated.`, [
        ...
        reactivity: section(`The usefulness of ${context.title} becomes clearer when it is tied to the processes that make Earth systems change, so this chapter follows the dynamic principles that govern geologic behavior.`, [
        ...
        occurrence: section(`${context.title} appears in natural systems, field settings, laboratory interpretation, and applied Earth science, so the chapter treats context as part of understanding rather than an afterthought.`, [
        ...
        isotopes: section(`Beyond definition and description, ${context.title} has a quantitative and theoretical side. This chapter explains the logic behind the calculations, time scales, measurement tools, and deeper principles that make the topic transferable.`, [
        ...
        production: section(`Students operationalize ${context.title} through field workflows, observation, data organization, and worked interpretation, so this chapter is organized around method rather than isolated fact recall.`, [
        ...
        applied: section(`${context.title} matters because it shapes how geologists reconstruct Earth history, assess hazards, locate resources, or interpret human interaction with the planet. The closing chapter ties the topic to practical use, broader significance, and safe professional judgment.`, [
    ...
    switch (key) {
        ...
        case 'physical':
            return [
                ...
                (fact) => `Representative observations show that ${fact}.`,
        ...
        case 'reactivity':
            return [
                ...
                (fact) => `Geologists rely on ${context.title} because ${fact}.`,
                (fact) => `A representative setting is ${fact}.`,
        ...
        case 'occurrence':
            return [
                ...
                (fact) => `A geologist is likely to meet the topic through ${fact}.`,
                (fact) => `Context matters because ${fact}.`,
                (fact) => `A simplified treatment is acceptable when ${fact}.`,
            ];
        case 'isotopes':
            return [
                (fact) => `Quantitative geology treats ${context.title} through ${fact}.`,
...
function chooseDistractors(context, key, index, allContexts) {
    ...
    const crossTopic = allContexts
        .filter((candidate) => candidate.id !== context.id && candidate.topicId !== context.topicId)
        .map((candidate) => candidate.sections[key].facts[index]);
    return unique([...sameTopic, ...crossTopic].filter((fact) => Boolean(fact))).slice(0, 3);
}
function buildQuiz(context, definition, section, allContexts) {
    ...
    const multipleChoiceQuestions = section.facts.map((fact, index) => multipleChoice(`${definition.id}-mc-${index + 1}`, prompts[index] ?? `Key fact ${index + 1} for ${context.title}`, fact, chooseDistractors(context, definition.key, index, allContexts), `The best answer is "${formatStandaloneText(fact)}" because ${(section.highlights[index % section.highlights.length] ?? section.focusNote).toLowerCase()}`, index));
...
function createUnitContext(topic, unit, topicIndex, index) {
    const order = index + 1;
...
function createGeologyUnit(context, allContexts) {
    return {
        id: context.id,
...
const geologyUnitContexts = geologyTopicSeeds.flatMap((topic, topicIndex) => topic.units.map((unit, index) => createUnitContext(topic, unit, topicIndex, index)));
exports.geologyAccentSources = Object.fromEntries(geologyUnitContexts.map((context) => {
    const accentSource = geologyAccentCycle[(context.order - 1 + (context.topicOrder - 1) * 3) % geologyAccentCycle.length];
    return [context.id, accentSource];
}));
exports.geologyTopics = geologyTopicSeeds.map((topic) => {
...
exports.geologyUnits = exports.geologyTopics.flatMap((topic) => topic.learningUnits);
